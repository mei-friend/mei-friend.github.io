...
- Create a new GitHub repository from the <a href="https://github.com/mei-friend/caller-template" target="_blank" rel="noopener noreferrer">caller template</a>. This sets up the necessary GitHub Actions workflow to connect your repository with the central repository.
...
Go to **Settings > mei-friend > Use GitHub Actions** and ensure that the box **"Show available GitHub Actions"** is checked.
...
- Open the **GitHub menu** in the menu bar and click **"GitHub Actions: Call work package workflow"**. If this button is not visible, verify that GitHub Actions is enabled in the settings and a JSON work package definition is provided (see [Enabling automation in mei-friend](#enabling-automation-in-mei-friend)).
...
The JSON file already contains the correct `central_repository`, `branch`, and `automation_path` values pointing to the E-LAUTE central repository. Once the URL is set, open any MEI file from your caller repository in mei-friend. The GitHub Actions panel will show the E-LAUTE work packages in the dropdown menu, ready to be applied to your encoding.
