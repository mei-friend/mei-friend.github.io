...
Provide the URL of a JSON file that defines the connection to the chosen central repository and the available work packages and their parameters in the **"Work package definition"** field, the following example file can be used for testing: [work_packages.json](https://raw.githubusercontent.com/mei-friend/automation/refs/heads/main/work_packages.json).
...
- Open the **GitHub menu** in the menu bar and click **"GitHub Actions: Call work package workflow"**. If this button is not visible, verify that GitHub Actions is enabled in the settings and a JSON work package definition is provided.
...
<figure class="halfwidth">
    ...
    <figcaption class="figure-caption">GitHub menu with activated GitHub Actions. Clicking on "GitHub Actions: Call work package workflow" opens the GitHub Actions panel.</figcaption>
</figure>

- In the GitHub Actions panel you should see a dropdown menu listing the available work packages defined in the JSON file.
...
<figure class="halfwidth">
    ...
    <figcaption class="figure-caption">GitHub Actions panel with filled in parameters of a chosen work package.</figcaption>
</figure>
...
The JSON work package definition file controls which work packages appear in mei-friend's GitHub Actions panel and how they are presented to the user. It is hosted at a publicly accessible, CORS-enabled URL — for example as a raw file in a GitHub repository — and referenced in mei-friend's settings (see [Enabling automation in mei-friend](#enabling-automation-in-mei-friend)).

Users are not required to inspect or understand the underlying scripts or YAML workflow definitions. A JSON template and a working example are provided in the central repository:
...
- **Work package definition**: defines available work packages and their parameters in a JSON file([Template](https://github.com/mei-friend/automation/blob/main/work_package_template.json), [Example](https://github.com/mei-friend/automation/blob/main/work_packages.json))
...
The caller repository is easiest set up from the [provided template](https://github.com/mei-friend/caller-template), which already includes this caller workflow.
The central repository, branch, and automation script are specified entirely through the work package definition file — switching to a different central repository is done by pointing mei-friend to a different JSON URL.
...
The central repository contains the actual processing logic: the scripts (e.g., shell or Python) that implement the work packages, and an automation entry-point script whose path is specified in the work package definition file.
...
2. The **caller workflow** uses the supplied `central_repository` and `branch` to check out the central repository and runs the script at the `automation_path`, passing all remaining inputs.
...
The E-LAUTE central repository provides a set of work packages tailored to the project's needs, pre- and post-processing, including validation against project-specific encoding guidelines.
...
In mei-friend's GitHub Actions panel, set the **"Work package definition"** URL to the E-LAUTE work package definition file:
...
The JSON file already contains the correct `central_repository`, `branch`, and `automation_path` values pointing to the E-LAUTE central repository.
