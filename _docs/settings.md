...
- **Speedmode:** Toggle Verovio Speed Mode. In Speedmode, only the current page is sent to Verovio to reduce rendering time with large files. See also [Working with large files: Speedmode](_docs/largefiles.md). This option can also be toggled the notation menu bar.
